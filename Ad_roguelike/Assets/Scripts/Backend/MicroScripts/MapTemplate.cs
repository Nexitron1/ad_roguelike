    ...
    public IEnumerator Anim()
    {
        ...
        for (int x = 0; x < 4; x++)
        {
            for (int y = 0; y < 4; y++)
            {
                ...
                {
                    Rooms[x, y] = (Instantiate(RoomPrefab, new Vector2(x, y) * transform.GetChild(1).localScale.x * 0.07f, Quaternion.identity, transform.GetChild(1))).GetComponent<Room>();
                    Rooms[x, y].transform.localPosition -= Vector3.one * 10.5f;                  
        ...
        for (int i = 0; i < generator.transitions1.Count; i++)
        {
            ...
            GameObject g = Instantiate(CorridorPrefab, transform.GetChild(1).localScale.x * 0.07f * new Vector2(generator.transitions1[i].x, generator.transitions1[i].y), q, transform.GetChild(1));
            g.transform.localPosition -= Vector3.one * 10.5f;
            yield return new WaitForSecondsRealtime(0.08f);
        }
